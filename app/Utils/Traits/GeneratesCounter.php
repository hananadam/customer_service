...
trait GeneratesCounter
{
        ...
        switch ($entity) {
            ...
            case Credit::class:
             ...
             case Reservation::class:
                $pattern_string = 'reservation_number_pattern';
                break;
        }
    ...
    private function getEntityCounter($entity, $client)
    {
        switch ($entity) {
            ...
            case Credit::class:
                ...
                break;
            case Reservation::class:
                if ($this->hasSharedCounterReservation($client, 'reservation'))
                    return 'invoice_number_counter';
     ...
     *
     * @return     string              The next reservation number.
     */
    public function getNextReservationNumber(Client $client, ?Reservation $reservation): string
    {
        $entity_number = $this->getNextEntityNumber(Reservation::class, $client);

        return $this->replaceUserVars($reservation, $entity_number);
     ...
     */ 
    public function hasSharedCounter(Client $client, string $type = 'credit'): bool
    {
        if ($type == 'credit')
     ...
     * Determines if it has shared counter.
     *
     * @param Reservation $reservation The client
     *
     * @return     bool             True if has shared counter, False otherwise.
     */ 
    public function hasSharedCounterReservation(Client $client, string $type = 'reservation'): bool
    {
        if ($type == 'reservation')
            return (bool)$client->getSetting('shared_invoice_reservation_counter');
    }
     ...
     *
     * @param $class
     * @param Collection $entity The entity ie App\Models\Client, Invoice etc
    ...
    private function resetCounters(Client $client)
    {
        ...
        $settings->reservation_number_counter = 1;
